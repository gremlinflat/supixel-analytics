...
#     new_img = img.unsqueeze(0).contiguous()
#     return new_img
...
# def post_process(img, pre_pad=0, scale=1):
#     _, _, h, w = img.size()
#     img = img[:, :, 0:h - pre_pad * scale, 0:w - pre_pad * scale]

#     new_img = torch.from_numpy(np.transpose(img.to('cpu'), (1, 2, 0))).float()
#     output = new_img.squeeze(0).contiguous()
...
# @torch.no_grad()
# def enhancer(img, model, outscale=1):
...
class RealESRGANer():
    ...
    def __init__(self,
        ...
        print(f'Using device: {self.device}') # logging
        ...
        if isinstance(model_path, list):
            ...
            if model_path.startswith('https://'):
                raise ValueError('Downloading models is not supported yet.')
    ...
    def pre_process(self, img):
        ...
        """
        img = torch.from_numpy(np.transpose(img, (2, 0, 1))).float()
        # self.img = img.unsqueeze(0).to(self.device) # <- original
...
if __name__ == '__main__':
    ...
    if args.model_name == 'srvgg':
        ...
        model_path = './FNX4_Generic_/FNX4_Generic_SRVGGNetCompact/models/net_g_latest.pth'
